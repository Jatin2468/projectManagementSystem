...
const ProjectList = () => {
  ...
  const searchProjectsData = project?.searchProjects || [];
  ...
  const uniqueProjects = (keyword ? searchProjectsData : projects).filter(
    (item, index, self) =>
      item && item.id && index === self.findIndex((p) => p.id === item.id)
  );
